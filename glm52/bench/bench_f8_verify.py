"""Wide re-tune of fusion #8 at specific batch sizes, for points the sweep may have
under-tuned.
...
from glm52.bench.bench_f8_sweep import SEED_SUM, DownProblem, E, H, I  # noqa: E402
...
def main() -> None:
    ...
    for T in [int(x) for x in args.batches.split(",")]:
        ...
        tu = autotune(lambda c: p.unfused(c, SEED_SUM)[:1], grid, warmup=4, rep=12)
        print(f"  unfused GEMM best {tu.best_ms:.4f} ({tu.n_tried - tu.n_failed} ok) "
              f"{tu.best_cfg}", flush=True)
        cs = _coord_tune(f"moe_sum T{T}", lambda c: [p.unfused(tu.best_cfg, c)[1]],
                         dict(SEED_SUM), rounds=1)
        tf = autotune(lambda c: p.fused(c), grid, warmup=4, rep=12)
        print(f"  fused   best {tf.best_ms:.4f} ({tf.n_tried - tf.n_failed} ok) "
              f"{tf.best_cfg}", flush=True)

        for fn in p.unfused(tu.best_cfg, cs):
            fn()
        torch.cuda.synchronize()
        idx, ref = p.reference()
        err_u = rel_err(p.routed[idx], ref)
        p.routed.zero_()
        for fn in p.fused(tf.best_cfg):
            fn()
        torch.cuda.synchronize()
        ...
        us, fs, gains = [], [], []
        for _ in range(5):
            a = bench_chain(p.unfused(tu.best_cfg, cs), warmup=8, rep=25).p50_ms
            b = bench_chain(p.fused(tf.best_cfg), warmup=8, rep=25).p50_ms
        ...
        row = dict(T=T, rows=p.rows, rows_per_expert=p.rows / E,
                   unfused_ms=us_s[2], fused_ms=fs_s[2], gain=g_s[2],
    ...
    (RESULTS / f"f8_sweep_{args.tag}.json").write_text(
        json.dumps({"id": f"f8_sweep_{args.tag}",
