...
fact (8 MB on C500, 32 MB on the RTX 4060, ~50 MB on the H200), so it comes from the probe
or the profile below, never from a literal.
...
# On Hopper the gap is expected to *reopen*, because cuBLAS uses TMA + warp specialization
# unconditionally while Triton only does so if the probe says those paths compile -- which
# is exactly why `preflight.py` records both numbers and why `c_peak_vendor` is carried
# alongside but never used in the arithmetic.
#
...
# 4.84 ms); RTX 4060: 139 GB/s copy against 161 GB/s read-only. We keep the conservative
...
# The H200's will be roughly 2x that -- Hopper's tensor cores scale further past HBM3e than
# either previous device's did -- so unlike the C500 -> 4060 step, the compute/memory labels
# do NOT carry over: cells near the old balance point flip to *memory*-bound here. That is a
# real device difference, not a modelling change, and the printed balance point is the
# number to quote when a label disagrees with LOG-09.
...
DEVICE_PEAKS: dict[str, DevicePeaks] = {
    ...
    # H200 (sm_90). *** THESE FOUR NUMBERS ARE ESTIMATES, NOT MEASUREMENTS. ***
    # Nobody on this side has an H200; the row exists so the model is self-consistent
    # before `preflight_h200.json` comes back, and `estimated=True` makes every consumer
    # able to say so. Basis, all order-of-magnitude:
    #   c_peak 600 TF/s   -- vendor dense bf16 spec is ~989 TF/s; Triton lands 60-75 % of
    #                        that on a large GEMM *with* TMA + warp specialization and
    #                        materially less without, and which of those compile is exactly
    #                        what the preflight probes. Expect +-30 %.
    #   b_peak 3.40 TB/s  -- HBM3e spec is 4.8 TB/s; measured mixed read+write streams
    #                        typically land ~0.70-0.75 of spec.
    #   b_peak_ro 4.0 TB/s-- read-only stream, ~0.83 of spec.
    #   l2 50 MB          -- Hopper's L2; the probe reports the exact figure and wins.
    # Anything printed against this row is a prediction. Run preflight.py and the measured
    # numbers replace all four (see `PEAKS.source` / `PEAKS.from_calibration`).
    "h200": DevicePeaks(
        "H200",
        600e12,
        3.40e12,
...
def _overlay_preflight(p: DevicePeaks, d: dict) -> tuple[DevicePeaks, list[str]]:
    ...
    Field mapping, with the reasoning for each choice:
      ...
      launch_s          <- `calibration.launch_us`
      timer_tick_s      <- `calibration.timer_tick_us`
...
def require_measured_peaks() -> DevicePeaks:
    ...
    against estimated peaks is a different act, and the report generators should call this
    first. The H200 row's four numbers are guesses until `preflight_h200.json` exists.
    """
