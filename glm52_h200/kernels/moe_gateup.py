...
this process may emit; this module only adapts.  The cfg keys the launcher forwards are
...
read at launch time:
  ...
  * The device's best bf16 Triton GEMM is BM128 BN256 BK64 GM8 w8 s3 (788 TF/s, 96 % of
    cuBLAS) and the per-block SMEM ceiling is 232448 B.  Under the corrected staging rule
    (see `smem_stage_bytes`) the UNFUSED arm models that shape at 147456 B and BM256 BN256
    BK64 s3 at 196608 B -- both fit.  The FUSED arm stages a second B tile, so at BN256 it
    needs 245760 B and is legitimately out; its reachable neighbours are BN128 shapes
    (BM128 BN128 BK64 s3 -> 147456 B).  That asymmetry is real work, not a pruning
    artefact: it is exactly the cost the fused variant pays for never materialising the
    4096-wide intermediate, and both arms are measured against the same device ceiling.
  ...
  `warp_specialize` (+ `num_consumer_groups` / `num_buffers_warp_spec` on forks using the
      launch-kwarg spelling -- measured DEAD on this H200's triton 3.6.0)
...
def clear_caches() -> None:
    """Drop memoized descriptors and flattened weight views.  Call when w13 is freed.

    Both memos hold a reference to the tensor they describe, so they keep 12 GB of expert
...
def _pipeline_buffers(num_stages: int) -> int:
    ...
    fairness check can see it -- but the error scales with the tile, so it is LARGER for
    the fused arm here (which stages two B tiles, `bn_mult=2`) than for the unfused one.
    A pruning rule whose error differs between the two arms is exactly what this study
    cannot detect after the fact, so the rule is read from the device at runtime.
    """
